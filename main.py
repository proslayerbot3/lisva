...
def onmessage(update,bot:ObigramClient):
    try:
        ...
        if username == tl_admin_user or user_info :  # validate user
            ...
            mensaje = "❌No tienes Acceso❌.\nPor favor Contacta con mi Programador @Frebootcar\n"
        ...
        if '/seteva' in msgText:
            ...
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,statInfo)
            return
        if '/setvcl' in msgText:
            getUser = user_info
            user = ''
            passw = ''
            hostmo = 'https://www.aula.vcl.sld.cu/'
            zips = 50
            repoid = 4
            uptype = 'calendar' 
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,statInfo)
            return
        if '/setsld' in msgText:
            getUser = user_info
            user = ''
            passw = ''
            hostmo = 'https://aulavirtual.hlg.sld.cu'
            zips = 20
            repoid = 4
            uptype = 'calendar' 
            if getUser:
        ...
        if '/setuvs' in msgText:
            ...
            user = ''
            passw = ''
            hostmo = 'https://uvs.ucm.cmw.sld.cu/'
            zips = 20
            repoid = 4
            uptype = 'calendar'
            if getUser:
                ...
                bot.sendMessage(update.message.chat.id,statInfo)
            return
        if '/setgtm' in msgText:
            getUser = user_info
            user = ''
            passw = ''
            hostmo = 'https://aulauvs.gtm.sld.cu/'
            zips = 7
            repoid = 4
            uptype = 'evidence' 
            if getUser:
...
def main():
    ...
    #set in debug
    bot_token = '6132606149:AAGtEj1X9wlZwRs8631cI14hp4lPxVLT1ac'
